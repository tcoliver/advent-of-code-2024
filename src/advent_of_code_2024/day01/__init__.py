...
def part_1():
    ...
    # Prepare input
    column_a, column_b = zip(*(line.split() for line in input.splitlines()))

    print("Total Distance: ", sum(abs(int(a) - int(b)) for a, b in zip(sorted(column_a), sorted(column_b))))
...
def part_2():
    ...
    counts_a = Counter(column_a)
    counts_b = Counter(column_b)

    print("Similarity Score: ", sum(int(k) * v * counts_b[k] for k, v in counts_a.items()))
