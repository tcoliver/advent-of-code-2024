...
def part_1():
    ...
    update being printed. Because you are currently only printing the correctly-ordered
    updates, you will need to find the middle page number of each correctly-ordered
    update. In the above example, the correctly-ordered updates are:
    ...
    rule_map = functools.reduce(
        lambda acc, rule: {**acc, rule[0]: acc.get(rule[0], set()) | {rule[1]}},
        rules,
        {},
    )
    update_map = [{page: i for i, page in enumerate(update)} for update in updates]

    correct = []
    for i, update in enumerate(update_map):
        for page in update:
            if page in rule_map and not all(
                update[page] < update[other] for other in rule_map[page] if other in update
            ):
                break
        else:
            correct.append(updates[i])

    print("Sum of centers: ", sum(map(lambda update: update[len(update) // 2], correct)))
...
def part_2():
    ...
    fixed = []
    for i in set(range(len(updates))) - correct:
        fixed.append(sorted(updates[i], key=functools.cmp_to_key(rule_compare)))

    print("Sum of centers: ", sum(map(lambda update: update[len(update) // 2], fixed)))
